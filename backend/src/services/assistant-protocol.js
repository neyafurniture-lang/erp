...
export const ACTION_CATALOG = [
  ...
  { type: 'update_task', usage: 'Modifier tâche (titre/statut)', params: { task_title: 'string', new_title: 'string?', status: 'done|doing|todo?', project_name: 'string?' } },
  ...
  { type: 'update_project', usage: 'Notes / deadline / budget / statut projet', params: { project_name: 'string?', project_id: 'number?', notes: 'string?', append_notes: 'boolean?', status: 'active|paused|done|cancelled?', deadline: 'YYYY-MM-DD?', budget_estimated: 'number?' } },
  { type: 'search_projects', usage: 'Chercher projets', params: { query: 'string', status: 'active|paused|done?' } },
  ...
  { type: 'add_project_material', usage: 'Ajouter un matériau au projet', params: { description: 'string', quantity: 'number?', project_id: 'number?', project_name: 'string?' } },
