...
async function resolveProjectId({ message, pageContext, params, study, keywords }) {
  ...
  if (q) {
    const { rows } = await pool.query(
      `SELECT id FROM projects WHERE status IN ('active','waiting','on_hold','paused') AND name ILIKE $1 ORDER BY priority DESC LIMIT 1`,
      [`%${q.split(/\s+/)[0]}%`]
    );
  ...
  for (const kw of keywords || []) {
    if (kw.length >= 4) {
      const { rows } = await pool.query(
        `SELECT id FROM projects WHERE status IN ('active','waiting','on_hold','paused') AND name ILIKE $1 LIMIT 1`,
        [`%${kw}%`]
      );
      if (rows[0]) return rows[0].id;
    }
