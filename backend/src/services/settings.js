...
const DEFAULTS = {
  ...
  /** Vide = non configuré ; le défaut historique 31250 n’est plus injecté ici (prod fail-closed). */
  project_admin_pin: '',
};
...
export async function getPublicSettings() {
  ...
  return {
    ...
    google_client_secret_preview: (all.google_client_secret || process.env.GOOGLE_CLIENT_SECRET)
      ...
      : '',
    project_admin_pin_configured: Boolean(
      String(all.project_admin_pin || '').trim()
      || String(process.env.FINANCE_SESSION_PIN || process.env.ADMIN_SESSION_PIN || process.env.PROJECT_ADMIN_PIN || '').trim()
    ),
