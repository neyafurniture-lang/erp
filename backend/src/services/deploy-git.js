...
export async function getGitDeployConfig() {
  ...
  return {
    ...
    vpsHost: process.env.NEYA_VPS_HOST || '51.222.31.75',
    autoDeployConfigured: Boolean(process.env.DEPLOY_HOST || process.env.NEYA_DEPLOY_SSH_HOST),
  };
}
 ...
 * Variables : NEYA_VPS_HOST, NEYA_VPS_USER, NEYA_VPS_SSH_KEY (chemin) ou NEYA_VPS_PASSWORD
 */
export function triggerVpsGitDeploy({ force = false } = {}) {
  const host = process.env.NEYA_VPS_HOST || process.env.DEPLOY_HOST;
  ...
  if (!host) {
    throw new Error('NEYA_VPS_HOST (ou DEPLOY_HOST) non configuré — impossible de joindre le VPS.');
  }
  ...
  // Si password uniquement (Windows/Linux avec sshpass)
  if (password && !key) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      else reject(new Error(stderr || stdout || `ssh exit ${code}`));
    });
  });
}
