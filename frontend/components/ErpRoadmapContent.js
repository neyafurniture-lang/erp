...
export default function ErpRoadmapContent() {
  ...
  return (
    <div className="space-y-8">
      <p className="text-sm text-neya-muted max-w-2xl neya-enter">
      ...
      {msg && <div className="text-sm bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-xl neya-enter">{msg}</div>}
      {err && (
        <div className="text-sm bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl neya-enter">
          {err}{' '}
          <Link href="/settings?tab=cursor" className="underline text-neya-orange">Configurer l&apos;agent</Link>
        </div>
      )}

      <section className="neya-enter" style={{ animationDelay: '45ms' }}>
        ...
        <ul className="space-y-2 neya-stagger">
          {PRIORITY.map(item => (
            <li key={item.id} className="neya-lift rounded-2xl border border-neya-border bg-neya-surface/60 px-4 py-3 flex flex-wrap items-start justify-between gap-3 shadow-sm">
      ...
      <section className="neya-enter" style={{ animationDelay: '90ms' }}>
        ...
        <ul className="grid sm:grid-cols-2 gap-2 text-sm text-neya-muted neya-stagger">
          {BACKLOG.map(item => (
            <li key={item} className="neya-lift flex gap-2 bg-white border border-neya-border rounded-2xl px-3 py-2 shadow-sm">
      ...
      <section className="neya-enter" style={{ animationDelay: '135ms' }}>
        <p className="text-[10px] uppercase tracking-wide text-green-700 font-semibold mb-3">Déjà livré</p>
        <ul className="grid sm:grid-cols-2 gap-2 text-sm text-neya-muted neya-stagger">
          {DONE.map(item => (
            <li key={item} className="neya-lift flex gap-2 bg-green-50/50 border border-green-100 rounded-2xl px-3 py-2 shadow-sm">
      ...
      <p className="text-xs text-neya-muted border-t border-neya-border pt-4 neya-enter" style={{ animationDelay: '180ms' }}>
